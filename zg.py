...
def get_config():
    ...
    if _config_cache is None:
        if not os.path.exists(CONFIG_FILE):
            ...
            print("Führe zuerst aus: python zg.py init <url> <user> <pass> <repo_id>")
            sys.exit(1)
...
def should_ignore(filepath):
    """ Linux-Native Ignore-Check, dynamisch gesteuert durch .zg_config.json """
    if "zg_temp_" in filepath:
        return True
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bin), 'r') as zf:
        ...
        if modified:
            ...
            for f in modified: 
                ...
                try:
                    sc_str = zf.read(f).decode('utf-8').splitlines()
                    lc_str = open(f, 'r', encoding='utf-8').read().splitlines()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("  init <url> <user> <pass> <repo>  - Verbindet diesen Ordner mit der Cloud")
